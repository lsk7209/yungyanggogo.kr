...
const fourthClusters = [
  ["oatmeal-fiber", "오트밀", "오트밀 식이섬유", "압착귀리", "1회분", "당류", "아침 식사 대용으로 양을 정해야 할 때"],
  ["instant-oat-cup-sugar", "컵오트", "컵오트 당류", "토핑", "분말스프", "우유 조리", "간편 오트 제품의 단맛을 확인할 때"],
  ["canned-tuna-sodium", "참치캔", "참치캔 나트륨", "기름", "수분", "단백질", "캔 하나를 반찬으로 볼지 식재료로 볼지 정할 때"],
  ["canned-corn-sugar", "옥수수캔", "옥수수캔 당류", "절임액", "총내용량", "나트륨", "샐러드 토핑으로 넣는 양을 볼 때"],
  ["pickle-sodium", "피클", "피클 나트륨", "절임류", "1회 섭취량", "당류", "곁들임 음식이라 수치를 가볍게 볼 때"],
  ["kimchi-sodium-label", "김치", "김치 나트륨 표시", "절임 배추", "양념", "100g 기준", "반찬 한 접시의 나트륨을 비교할 때"],
  ["ready-side-dish-sodium", "반찬 간편식", "반찬 간편식 나트륨", "조림", "볶음", "총내용량", "소량 반찬 여러 개를 같이 먹을 때"],
  ["instant-udon-sodium", "즉석우동", "즉석우동 나트륨", "국물", "면 중량", "소스", "국물을 남길지 포함해서 볼지 고민될 때"],
  ["cold-noodle-sauce-sugar", "냉면소스", "냉면소스 당류", "육수", "비빔장", "나트륨", "면보다 소스가 수치를 바꿀 때"],
  ["noodle-topping-calorie", "면 토핑", "면 토핑 열량", "튀김고명", "건더기스프", "기름", "라면이나 우동에 추가 토핑을 넣을 때"],
  ["pudding-sugar", "푸딩", "푸딩 당류", "디저트컵", "총내용량", "포화지방", "작은 디저트컵의 당류를 확인할 때"],
  ["jelly-drink-sugar", "젤리음료", "젤리음료 당류", "파우치", "100ml 기준", "열량", "음료와 간식 사이 제품을 비교할 때"],
  ["sports-drink-sugar", "스포츠음료", "스포츠음료 당류", "전해질", "500ml 용량", "열량", "운동 후 마시는 음료의 총당류를 볼 때"],
  ["coconut-water-sugar", "코코넛워터", "코코넛워터 당류", "100ml 기준", "칼륨", "용량 차이", "건강한 음료 이미지와 당류를 분리할 때"],
  ["tofu-snack-protein", "두부과자", "두부과자 단백질", "과자류", "지방", "나트륨", "두부 이름 때문에 단백질을 기대할 때"],
  ["rice-cake-snack-carbs", "떡 간식", "떡 간식 탄수화물", "소포장", "당류", "총열량", "한입 떡을 여러 개 먹을 때"],
  ["dried-fruit-sugar", "건과일", "건과일 당류", "망고", "크랜베리", "총내용량", "과일 간식이라 당류를 낮게 예상할 때"],
  ["cereal-milk-combo", "시리얼 우유 조합", "시리얼 우유 조합 열량", "우유 200ml", "당류", "단백질", "시리얼만 볼 때와 우유를 더할 때 차이를 볼 때"],
  ["soup-base-sodium", "육수팩", "육수팩 나트륨", "농축액", "희석비율", "국물 요리", "조리 후 실제 먹는 국물량을 계산할 때"],
  ["meal-replacement-drink", "식사대용 음료", "식사대용 음료 영양성분", "단백질", "당류", "식이섬유", "바쁜 날 한 끼 대체로 마셔도 되는지 볼 때"]
];
...
const batches = [
  ...
  {
    id: "2026-08-08",
    outFile: "scheduled-2026-08-08.json",
    manifestFile: "blog-batch-manifest-2026-08-08.json",
    startUtc: Date.UTC(2026, 7, 8, 3, 0, 0),
    clusters: fourthClusters,
    structureOffset: 17
  }
];
